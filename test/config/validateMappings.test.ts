...
function writeYamlMapping(dir: string, name: string, data: unknown): string {
  const mappingPath = join(dir, name);
  writeFileSync(mappingPath, stringify(data, { lineWidth: 0 }) + '\n');
  return mappingPath;
}
...
describe('E7.T4 — validateIngressMapping', () => {
  it('passes for Scenario-03 ingress mapping', () => {
    ...
    expect(() =>
      validateIngressMapping(join(SCENARIO_03, 'mapping/ingress.mqtt.yaml'), tree, 'mqtt-ingress'),
    ).not.toThrow();
  });

  it('throws ConfigError when ingress targets a missing node', () => {
    const dir = mkdtempSync(join(tmpdir(), 'ingress-val-'));
    const mappingPath = writeYamlMapping(dir, 'bad-ingress.yaml', {
      version: 1,
      rules: [{
  ...
  it('throws ConfigError when ingress targets a missing property', () => {
    const dir = mkdtempSync(join(tmpdir(), 'ingress-val-'));
    const mappingPath = writeYamlMapping(dir, 'bad-prop.yaml', {
      version: 1,
      rules: [{
        match: { topicFilter: 'devices/x/status' },
        target: {
...
describe('E7.T4 — validateEgressMapping', () => {
  it('passes for Scenario-03 egress mapping', () => {
    ...
    expect(() =>
      validateEgressMapping(
        join(SCENARIO_03, 'mapping/egress.is12.yaml'),
        tree,
        entities,
        'is12-egress',
      ),
  ...
  it('throws ConfigError when egress instance location is missing from tree', () => {
    ...
    const mappingPath = writeYamlMapping(dir, 'bad-instance.yaml', {
      version: 1,
  ...
  it('throws ConfigError when egress mapping has property gaps', () => {
    ...
    const mappingPath = writeYamlMapping(dir, 'gappy.yaml', {
      version: 1,
      classes: [{

...
// ---------------------------------------------------------------------------
// E3.T1 — validateBaseTypeValue — base type validation (migrated from E1.T5)
// Note: more exhaustive per-type tests live in BaseType.test.ts.
// ---------------------------------------------------------------------------
...
describe('E3.T1 — validateBaseTypeValue — numeric', () => {
  it('accepts finite numbers (integers and floats)', () => {
    expect(validateBaseTypeValue('numeric', 0).ok).toBe(true);
    expect(validateBaseTypeValue('numeric', 3.14).ok).toBe(true);
    expect(validateBaseTypeValue('numeric', -1e300).ok).toBe(true);
    expect(validateBaseTypeValue('numeric', 1000000).ok).toBe(true);
  });
  it('rejects NaN', () => {
  ...
  it('rejects a boolean', () => {
    const r = validateBaseTypeValue('numeric', true);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error.kind).toBe('wrong-type');
  });
});
...
describe('E3.T1 — validateBaseTypeValue — object', () => {
  ...
  it('rejects an array', () => {
    const r = validateBaseTypeValue('object', [1, 2]);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error.kind).toBe('wrong-type');
  });
  it('rejects a string', () => {
...
describe('E3.T1 — no old int/float width kinds', () => {
  it('only the four base types exist at runtime', () => {
    const baseTypes = ['string', 'bool', 'numeric', 'object'] as const;
    for (const t of baseTypes) {
      expect(validateBaseTypeValue(t, t === 'string' ? '' : t === 'bool' ? true : t === 'numeric' ? 0 : {}).ok).toBe(true);
    }
  });
});
